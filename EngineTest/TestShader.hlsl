...
#define ElementsTypeSkeletalNormalTextureColor  ElementsTypeSkeletalNormalTexture | ElementsTypeStaticNormal
...
float3 PointLight(float3 N, float3 worldPosition, float3 V, LightParameters light)
{
    ...
    if (dSq < light.Range * light.Range)
    {
        ...
        color = CalculateLighting(N, L, V, light.Color * attenuation * light.Intensity * 0.2f);
    }
#endif
    return color;
}
