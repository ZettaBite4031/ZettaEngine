...
groupshared uint        _spotlightStartOffset;             // self explanatory
...
Sphere GetBoundingSphere(float3 tip, float range, float3 direction, float cosPenumbra)
{
    Sphere sphere;
...
void CullLightsCS(ComputeShaderInput csIn)
{
    ...
    const float3 pos = AntiprojectionUV(csIn.DispatchThreadID.xy * invViewDimensions, depth, GlobalData.InvViewProjection).xyz;
    ...
    for (i = 0; i < lightCount; i++)
    {
        ...
        {
            // NOTE: -1 meant the light isa point light; otherwise, it's a spotlight.
